/**
 * scripts/confirmed-retest-v2-study.ts — historical replay for
 * `xauusd-h4-confirmed-retest-v2` (spec: src/research/confirmed-retest-v2/).
 *
 ...
 * self-describing run directory and updates `latest.json`. Structurally
 * identical to `confirmed-retest-study.ts` (v1) — one run, no grid search,
 * no automatic v3.
 *
 * Run:  npm run confirmed-retest-v2:study [-- --end 2026-09-11T20:00:00Z] [-- --out <dir>]
 */
...
import { latestCompletedM1CloseUtc } from '../src/research/confirmed-retest-v2/data-source';
import { executeRun, loadAll } from '../src/research/confirmed-retest-v2/pipeline';
import { writeRunArtifacts } from '../src/research/confirmed-retest-v2/report';
import { SPEC, SPEC_HASH } from '../src/research/confirmed-retest-v2/spec';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const runCommand = `npm run confirmed-retest-v2:study -- --end ${new Date(endT).toISOString()}`;
    writeRunArtifacts(outDir, run, {
      runId,
      runCommand,
      generatedAtUtc: new Date().toISOString(),
      gitCommit: git('rev-parse HEAD'),
      gitDirty: git('status --porcelain -- src/research/confirmed-retest-v2 scripts') !== '',
