...
const config: AiConfig = {
  ...
  apiKey: 'sk-or-test-secret-12345',
...
describe('AutonomousOpenRouterProvider', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('parses a valid message-content response into a RawAutonomousAiDecision', async () => {
    mockOpenRouterResponse(200, { choices: [{ message: { content: JSON.stringify(VALID_RAW_DECISION) } }] });
    const provider = new AutonomousOpenRouterProvider(config);
    await expect(provider.decide(context)).resolves.toEqual(VALID_RAW_DECISION);
  });

  it('sends temperature: 0 and the autonomous system prompt', async () => {
    const fetchSpy = mockOpenRouterResponse(200, { choices: [{ message: { content: JSON.stringify(VALID_RAW_DECISION) } }] });
    const provider = new AutonomousOpenRouterProvider(config);
  ...
  it('throws on a non-2xx response', async () => {
    mockOpenRouterResponse(429, { error: { message: 'rate limited' } });
    const provider = new AutonomousOpenRouterProvider(config);
    await expect(provider.decide(context)).rejects.toThrow(/429/);
  });

  it('throws on a 200 response carrying a top-level embedded error (free-tier routing failure)', async () => {
    mockOpenRouterResponse(200, { error: { message: 'Upstream error from Nvidia: Service temporarily overloaded' } });
    const provider = new AutonomousOpenRouterProvider(config);
    await expect(provider.decide(context)).rejects.toThrow(/Upstream error/);
  });
  ...
  it('times out and rejects if the request takes longer than requestTimeoutMs', async () => {
    ...
    const provider = new AutonomousOpenRouterProvider({ ...config, requestTimeoutMs: 50 });
    await expect(provider.decide(context)).rejects.toThrow();
  });
});
