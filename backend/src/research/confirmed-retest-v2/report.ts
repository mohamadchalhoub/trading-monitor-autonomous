...
export function renderReport(run: RunOutputs, extras: ManifestExtras): string {
  ...
  const lines = [
    ...
    `Of qualified pivots watched for a retest — body violation before/during watch: R ${d.bodyViolationDuringWatch.RESISTANCE} / S ${d.bodyViolationDuringWatch.SUPPORT}; no retest within the 5..120 window: R ${d.noRetestWithinWindow.RESISTANCE} / S ${d.noRetestWithinWindow.SUPPORT}; retest found: R ${d.retestFound.RESISTANCE} / S ${d.retestFound.SUPPORT}.`,
    `Of retests found — confirmation failed (none of R/R+1/R+2 closed $10 favorably, retired): R ${d.confirmationFailedAfterRetest.RESISTANCE} / S ${d.confirmationFailedAfterRetest.SUPPORT}; activated: R ${d.levelsActivated.RESISTANCE} / S ${d.levelsActivated.SUPPORT}.`,
