/**
 * research/confirmed-retest-v2/report — machine-readable run artifacts (JSON,
...
export function levelRows(levels: Level[]) {
  return levels.map((l) => ({
    ...
    pivotId: l.pivotId,
    pivotUtc: iso(l.pivotT),
    retestId: l.retestId,
    retestUtc: iso(l.retestT),
    confirmationUtc: iso(l.confirmationT),
...
function retestRows(retests: Array<{ id: string; index: number; barT: number; confirmT: number }>) {
  return retests.map((r) => ({ id: r.id, h4Index: r.index, barOpenUtc: iso(r.barT), confirmedAtUtc: iso(r.confirmT) }));
}
...
export function renderReport(run: RunOutputs, extras: ManifestExtras): string {
  ...
  const lines = [
    ...
    '',
    `H4 bars processed ${d.h4BarsProcessed}; pivot candidates R ${d.pivotCandidates.RESISTANCE} / S ${d.pivotCandidates.SUPPORT}; qualified (≥$10 rejection close on the 2 confirming bars) R ${d.qualifiedPivots.RESISTANCE} / S ${d.qualifiedPivots.SUPPORT} (not qualified: R ${d.rejectedNoRejectionClose.RESISTANCE} / S ${d.rejectedNoRejectionClose.SUPPORT}).`,
    `Of qualified pivots, how the retest SEARCH ended — body violation before any retest was found: R ${d.bodyViolationBeforeRetest.RESISTANCE} / S ${d.bodyViolationBeforeRetest.SUPPORT}; no retest within the 5..120 window: R ${d.noRetestWithinWindow.RESISTANCE} / S ${d.noRetestWithinWindow.SUPPORT}; retest found: R ${d.retestFound.RESISTANCE} / S ${d.retestFound.SUPPORT}. (The three do not always sum to "qualified": ${run.state.levels.watchList.length} pivot(s) were still mid-watch, unresolved either way, at this run's frozen endpoint — correctly, since resolving them needs bars this run does not have.)`,
    `Of retests found, how the CONFIRMATION window ended — body violation during R+1/R+2: R ${d.bodyViolationDuringConfirmation.RESISTANCE} / S ${d.bodyViolationDuringConfirmation.SUPPORT}; confirmation failed (none of R/R+1/R+2 closed $10 favorably, retired): R ${d.confirmationFailedAfterRetest.RESISTANCE} / S ${d.confirmationFailedAfterRetest.SUPPORT}; a qualifying close was found (activated, or blocked only by same-price dedup): R ${d.levelsActivated.RESISTANCE} / S ${d.levelsActivated.SUPPORT} activated.`,
    `Activations blocked by same-price/same-role dedup: ${JSON.stringify(d.activationsBlocked)}.`,
    `Levels activated total: R ${d.levelsActivated.RESISTANCE} / S ${d.levelsActivated.SUPPORT} (${studyLevels.length} activated inside the study period, ${activeAtStudyStart} active at study start); D1-agreement tagged ${d.d1AgreementTagged}.`,
...
export function writeRunArtifacts(dir: string, run: RunOutputs, extras: ManifestExtras): void {
  ...
  const manifest = {
    ...
    files: ['manifest.json', 'spec.json', 'coverage.json', 'formation.json', 'levels.json', 'levels.csv', 'pivots.csv', 'retests.csv', 'events.json', 'events.csv', 'event-study.json', 'paper-summary.json', 'paper-branches.json', 'REPORT.md'],
  };
  ...
  writeAtomic(join(dir, 'retests.csv'), csv(retestRows(run.state.levels.retestLog)));
